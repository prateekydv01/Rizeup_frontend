...
import { getGoalById, markGoalComplete, verifyProof, getGoalLeaderboard, updateGoal } from "../../api/goal.js";
...
const TabBtn = ({ active, onClick, children, badge }) => (
  <button onClick={onClick} style={{ padding: "9px 16px", background: "none", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", textTransform: "capitalize", color: active ? "#f97316" : "#52525b", borderBottom: active ? "2px solid #f97316" : "2px solid transparent", marginBottom: -1, transition: "color 0.15s", display: "flex", alignItems: "center", gap: 5 }}>
    {children}
...
function OverviewTab({ goal, currentUserId, onGoalUpdate }) {
  ...
  const isCompleted = goal.completedBy?.some(c => c.userId?._id === currentUserId || c.userId === currentUserId);
  const isPending   = goal.completionRequests?.some(r => (r.userId?._id === currentUserId || r.userId === currentUserId) && r.status === "pending");
  ...
  const InputS = {
    background: "#0d0d0f", border: "1px solid rgba(39,39,42,0.9)", color: "#f4f4f5",
    borderRadius: 8, padding: "9px 12px", fontSize: 13, fontFamily: "'DM Sans',sans-serif",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* Goal info / edit */}
      <div style={{ padding: "16px", borderRadius: 12, background: "rgba(15,15,17,0.8)", border: "1px solid rgba(39,39,42,0.7)" }}>
        {editing ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div>
              <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", color: "#52525b", marginBottom: 5 }}>Title</p>
              <input value={editTitle} onChange={e => setEditTitle(e.target.value)} style={InputS}
                onFocus={e => e.currentTarget.style.borderColor = "rgba(249,115,22,0.5)"}
                onBlur={e => e.currentTarget.style.borderColor = "rgba(39,39,42,0.9)"} />
            </div>
            <div>
              <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", color: "#52525b", marginBottom: 5 }}>End Date</p>
              <input type="date" value={editEnd} onChange={e => setEditEnd(e.target.value)} style={InputS}
                onFocus={e => e.currentTarget.style.borderColor = "rgba(249,115,22,0.5)"}
                onBlur={e => e.currentTarget.style.borderColor = "rgba(39,39,42,0.9)"} />
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setEditing(false)} style={{ flex: 1, padding: "8px", borderRadius: 8, border: "1px solid rgba(63,63,70,0.6)", background: "transparent", color: "#71717a", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
              <button onClick={handleSaveEdit} disabled={saving} style={{ flex: 2, padding: "8px", borderRadius: 8, border: "none", background: "linear-gradient(135deg,#f97316,#dc2626)", color: "white", fontSize: 12, fontWeight: 600, cursor: "pointer", opacity: saving ? 0.5 : 1 }}>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              <h3 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 900, fontSize: 16, color: "#f4f4f5", margin: 0 }}>{goal.title}</h3>
              <button onClick={() => setEditing(true)} style={{ fontSize: 10, fontWeight: 700, color: "#52525b", background: "rgba(39,39,42,0.5)", border: "1px solid rgba(63,63,70,0.4)", padding: "3px 10px", borderRadius: 6, cursor: "pointer", letterSpacing: "0.08em", transition: "all 0.15s" }}
      ...
      {/* Error / success */}
      {error && (
        <div style={{ padding: "10px 14px", borderRadius: 10, background: "rgba(220,38,38,0.07)", border: "1px solid rgba(220,38,38,0.2)", color: "#f87171", fontSize: 12 }}>{error}</div>
      )}
      {success && (
        <div style={{ padding: "10px 14px", borderRadius: 10, background: "rgba(34,197,94,0.07)", border: "1px solid rgba(34,197,94,0.2)", color: "#22c55e", fontSize: 12 }}>{success}</div>
      )}
          ...
          {goal.type === "circle" && (
            <div style={{ marginBottom: 10 }}>
              <input value={proof} onChange={e => setProof(e.target.value)} placeholder="Paste proof URL (screenshot, video, doc…)"
                style={{ background: "#0d0d0f", border: "1px solid rgba(39,39,42,0.9)", color: "#f4f4f5", borderRadius: 8, padding: "10px 13px", fontSize: 13, fontFamily: "'DM Sans',sans-serif", outline: "none", width: "100%", boxSizing: "border-box", transition: "border-color 0.2s" }}
                onFocus={e => e.currentTarget.style.borderColor = "rgba(249,115,22,0.5)"}
                onBlur={e => e.currentTarget.style.borderColor = "rgba(39,39,42,0.9)"} />
              <p style={{ fontSize: 10, color: "#3f3f46", marginTop: 5 }}>More than 50% of circle members must approve.</p>
            </div>
          )}
          <button onClick={handleComplete} disabled={submitting || (goal.type === "circle" && !proof.trim())}
            style={{ width: "100%", padding: "11px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#22c55e,#16a34a)", color: "white", fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer", opacity: submitting || (goal.type === "circle" && !proof.trim()) ? 0.45 : 1, transition: "opacity 0.2s" }}>
      ...
      {isPending && (
        <div style={{ padding: "14px 16px", borderRadius: 12, background: "rgba(234,179,8,0.05)", border: "1px solid rgba(234,179,8,0.15)", display: "flex", gap: 10, alignItems: "flex-start" }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: "#52525b", margin: 0 }}>Your proof has been submitted. Circle members are reviewing it.</p>
          </div>
        </div>
      )}
      ...
      {goal.status === "backlog" && !isCompleted && (
        <div style={{ padding: "14px 16px", borderRadius: 12, background: "rgba(113,113,122,0.07)", border: "1px solid rgba(113,113,122,0.2)", display: "flex", gap: 10, alignItems: "flex-start" }}>
          ...
          <div>
            <p style={{ fontSize: 12, fontWeight: 700, color: "#71717a", margin: "0 0 3px" }}>Goal moved to backlog</p>
            <p style={{ fontSize: 11, color: "#52525b", margin: 0 }}>The deadline passed. You can still complete it.</p>
          </div>
        </div>
      )}
    </div>
...
function ResourcesTab({ resources }) {
  if (!resources?.length) return (
    <div style={{ textAlign: "center", padding: "40px 0" }}>
      <p style={{ fontSize: 14, color: "#3f3f46" }}>No resources attached.</p>
    </div>
  );
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {resources.map((r, i) => (
        <a key={i} href={r.url} target="_blank" rel="noreferrer"
          style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 12, background: "rgba(15,15,17,0.8)", border: "1px solid rgba(39,39,42,0.7)", textDecoration: "none", transition: "all 0.15s" }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(249,115,22,0.35)"; e.currentTarget.style.background = "rgba(249,115,22,0.04)"; }}
          onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(39,39,42,0.7)"; e.currentTarget.style.background = "rgba(15,15,17,0.8)"; }}>
          <div style={{ width: 38, height: 38, borderRadius: 10, background: "rgba(249,115,22,0.1)", border: "1px solid rgba(249,115,22,0.18)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 17 }}>🔗</div>
          ...
          <span style={{ color: "#52525b", flexShrink: 0, fontSize: 13 }}>↗</span>
        </a>
      ))}
    </div>
  );
...
function VerificationsTab({ goal, currentUserId, onGoalUpdate }) {
  ...
  // Only show OTHER members' requests (not mine)
  const requests = goal.completionRequests?.filter(r => {
    const reqUserId = r.userId?._id || r.userId;
    return reqUserId?.toString() !== currentUserId?.toString();
  }) || [];
  ...
  if (!requests.length) return (
    <div style={{ textAlign: "center", padding: "40px 0" }}>
      <p style={{ fontSize: 14, color: "#3f3f46" }}>No pending verification requests.</p>
    </div>
  );
          ...
          <div key={req._id || i} style={{ borderRadius: 12, overflow: "hidden", border: `1px solid ${req.status === "approved" ? "rgba(34,197,94,0.2)" : req.status === "rejected" ? "rgba(239,68,68,0.15)" : "rgba(234,179,8,0.2)"}`, background: "rgba(8,8,10,0.7)" }}>
            ...
            <div style={{ padding: "14px 16px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                <div>
                  <p style={{ fontSize: 13, fontWeight: 700, color: "#f4f4f5", margin: "0 0 3px" }}>
                    {req.userId?.fullName || req.userId?.username || `Member #${i + 1}`}
                  </p>
                  <p style={{ fontSize: 10, color: "#52525b", margin: 0 }}>Submitted {fmtFull(req.createdAt)}</p>
                </div>
                <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", padding: "3px 9px", borderRadius: 20, background: `${statusColor}15`, border: `1px solid ${statusColor}40`, color: statusColor }}>
                  {req.status}
                </span>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: req.status === "pending" ? 12 : 0 }}>
                ...
                <span style={{ fontSize: 10, color: "#3f3f46" }}>›50% needed</span>
              </div>

              {req.status === "pending" && (
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => handleVote(req._id, "approve")} disabled={voting[req._id]}
                    style={{ flex: 1, padding: "8px", borderRadius: 8, border: "1px solid rgba(34,197,94,0.25)", background: "rgba(34,197,94,0.07)", color: "#22c55e", fontSize: 12, fontWeight: 700, cursor: "pointer", opacity: voting[req._id] ? 0.5 : 1, transition: "opacity 0.15s" }}>
                    ✓ Approve
                  </button>
                  <button onClick={() => handleVote(req._id, "reject")} disabled={voting[req._id]}
                    style={{ flex: 1, padding: "8px", borderRadius: 8, border: "1px solid rgba(239,68,68,0.18)", background: "rgba(239,68,68,0.06)", color: "#f87171", fontSize: 12, fontWeight: 700, cursor: "pointer", opacity: voting[req._id] ? 0.5 : 1, transition: "opacity 0.15s" }}>
                    ✕ Reject
                  </button>
                </div>
              )}
...
export default function GoalDetailModal({ goal: initialGoal, onClose, onUpdate }) {
  ...
  // Get current user ID from goal data
  const currentUserId = goal.createdBy?._id || goal.createdBy;

  // Fetch full goal on mount to get populated data
  useEffect(() => {
  ...
  const pendingVerifications = goal.completionRequests?.filter(r => {
    const reqId = r.userId?._id || r.userId;
    return reqId?.toString() !== currentUserId?.toString() && r.status === "pending";
  }).length || 0;
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, background: "rgba(0,0,0,0.88)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)" }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ width: "100%", maxWidth: 580, background: "linear-gradient(160deg,#111113,#0d0d0f)", border: "1px solid rgba(39,39,42,0.9)", borderRadius: 18, overflow: "hidden", maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 30px 80px rgba(0,0,0,0.7)" }}>
        ...
        <div style={{ padding: "20px 24px 0", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 16 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap", marginBottom: 5 }}>
                <h2 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 900, fontSize: 18, color: "#f4f4f5", margin: 0, letterSpacing: "-0.01em" }}>{goal.title}</h2>
                {goal.type === "circle" && (
          ...
          {/* Tabs */}
          <div style={{ display: "flex", borderBottom: "1px solid rgba(39,39,42,0.7)" }}>
        ...
        <div style={{ overflowY: "auto", padding: "18px 24px 24px", flex: 1 }}>
          {loading ? (
            ...
            <>
              ...
              {activeTab === "resources"     && <ResourcesTab      resources={goal.resources} />}
