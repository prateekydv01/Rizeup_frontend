...
const IS = { width:"100%", background:"#0d0d0f", border:"1px solid rgba(39,39,42,0.9)", color:"#f4f4f5", borderRadius:10, padding:"11px 14px", fontSize:13, fontFamily:"'DM Sans',sans-serif", outline:"none", boxSizing:"border-box", transition:"border-color 0.2s, box-shadow 0.2s" };
...
export default function CreateGoalModal({ onClose, onCreated }) {
  ...
  useEffect(() => { getUserCircles().then(r => setCircles(r.data.data)).catch(console.log); }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await createGoal({ title, startDate, endDate, type, circleId:type==="circle"?circleId:undefined, resources:resources.filter(r=>r.name.trim()&&r.url.trim()) });
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:16, background:"rgba(0,0,0,0.85)", backdropFilter:"blur(8px)" }}
      ...
      <div style={{ width:"100%", maxWidth:500, background:"linear-gradient(145deg,#111113,#0d0d0f)", border:"1px solid rgba(39,39,42,0.9)", borderRadius:18, overflow:"hidden", maxHeight:"92vh", display:"flex", flexDirection:"column", boxShadow:"0 24px 60px rgba(0,0,0,0.6)" }}>
        ...
        <div style={{ padding:"0 24px 24px", overflowY:"auto", flex:1 }}>
          {error && (
            <div style={{ marginBottom:16, padding:"10px 14px", borderRadius:10, background:"rgba(220,38,38,0.08)", border:"1px solid rgba(220,38,38,0.2)", color:"#f87171", fontSize:12, display:"flex", alignItems:"center", gap:8 }}>
              <svg width="13" height="13" viewBox="0 0 12 12" fill="none" style={{ flexShrink:0 }}><circle cx="6" cy="6" r="5.5" stroke="#f87171" strokeWidth="1"/><path d="M6 3.5v3M6 8.5h.01" stroke="#f87171" strokeWidth="1.2" strokeLinecap="round"/></svg>
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:18 }}>
            ...
            <div>
              ...
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Learn MERN Stack"
                autoFocus required style={IS} onFocus={fo} onBlur={bl} />
            </div>
              ...
              <div style={{ display:"flex", gap:8 }}>
                {["personal","circle"].map(t => (
                  <button key={t} type="button" onClick={() => { setType(t); if (t==="personal") setCircleId(""); }}
                    style={{ flex:1, padding:10, borderRadius:10, fontSize:12, fontWeight:600, cursor:"pointer", transition:"all 0.18s", textTransform:"capitalize", background:type===t?"rgba(249,115,22,0.12)":"rgba(15,15,17,0.8)", border:type===t?"1px solid rgba(249,115,22,0.45)":"1px solid rgba(39,39,42,0.9)", color:type===t?"#f97316":"#71717a" }}>
            ...
            {type==="circle" && (
              <div>
                ...
                {circles.length===0
                  ...
                  : <select value={circleId} onChange={e => setCircleId(e.target.value)} required style={{ ...IS, cursor:"pointer" }} onFocus={fo} onBlur={bl}>
            ...
            <div>
              ...
              {resources.length===0 && (
                <div style={{ padding:14, borderRadius:10, border:"1px dashed rgba(39,39,42,0.7)", display:"flex", alignItems:"center", justifyContent:"center" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {resources.map((r, i) => (
                  <div key={i} style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
                    <input value={r.name} onChange={e => updateRes(i,"name",e.target.value)} placeholder="Name"
                      style={{ ...IS, flex:"1 1 100px", fontSize:12, padding:"9px 12px" }} onFocus={fo} onBlur={bl} />
                    <input value={r.url}  onChange={e => updateRes(i,"url", e.target.value)} placeholder="https://…"
                      style={{ ...IS, flex:"2 1 140px", fontSize:12, padding:"9px 12px" }} onFocus={fo} onBlur={bl} />
            ...
            {type==="circle" && (
              <div style={{ padding:"12px 14px", borderRadius:10, background:"rgba(249,115,22,0.05)", border:"1px solid rgba(249,115,22,0.12)", display:"flex", gap:8 }}>
                ...
                <p style={{ fontSize:12, color:"#a16207", lineHeight:1.6, margin:0 }}>Circle goals require peer verification. Submit proof when marking complete.</p>
              </div>
            )}

            <button type="submit" disabled={loading}
              style={{ padding:13, borderRadius:12, border:"none", background:loading?"rgba(249,115,22,0.4)":"linear-gradient(135deg,#f97316,#dc2626)", color:"white", fontFamily:"'DM Sans',sans-serif", fontSize:14, fontWeight:600, cursor:loading?"not-allowed":"pointer", transition:"opacity 0.2s" }}
